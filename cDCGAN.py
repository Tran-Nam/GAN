...
G_input = tf.placeholder(tf.float32, shape=[None, 1, 1, noise_dim])
D_input = tf.placeholder(tf.float32, shape=[None, 28, 28, 1])
y = tf.placeholder(tf.float32, shape=[None, 1, 1, 10])
y_fill = tf.placeholder(tf.float32, shape=[None, 28, 28, 10])
...
def sample_Z(n_sample, n_dim):
    return np.random.uniform(-1., 1., size=[n_sample, 1, 1, n_dim])

def GNet(Z, y, reuse=False):
    with tf.variable_scope("GNet", reuse=reuse):
        a = tf.concat([Z, y], axis=3)
        h1 = tf.layers.dense(a, units=7*7*128)
...
def DNet(X, y_fill, reuse=False):
    # X = tf.reshape(X, [-1, 28, 28, 1])
    with tf.variable_scope("DNet", reuse=reuse):
        a = tf.concat([X, y_fill], axis=3)
        h1 = tf.layers.conv2d(a, filters=64, kernel_size=5, strides=2, padding='same')
        h1 = tf.layers.batch_normalization(h1, training=is_training)
...
r_img = DNet(D_input, y_fill)
f_img = DNet(G_sample, y_fill, reuse=True)
...
with tf.Session() as sess:
    ...
    for i in range(5000):
        ...
        y_batch = np.reshape(y_batch, newshape=[-1, 1, 1, 10])
        y_batch_fill = y_batch * np.ones([batch_size, 28, 28, 10])
        X_batch = X_batch * 2. - 1
        Z_batch = sample_Z(batch_size, noise_dim)

        _, loss_D, _ = sess.run([D_step, D_loss, clip_D], feed_dict={D_input: X_batch, G_input: Z_batch, y_fill: y_batch_fill, y: y_batch, is_training: True})

        # if i%5==0:
        y_ = np.random.randint(0, 9, (batch_size, 1))
        y_batch = onehot[y_.astype(np.int32)].reshape(batch_size, 1, 1, 10)
        # print(y_batch.shape)
        y__batch_fill = y * np.ones([batch_size, 28, 28, 10])
        # print(y_batch_fill.shape)
        Z_batch = sample_Z(batch_size, noise_dim)
        _, loss_G = sess.run([G_step, G_loss], feed_dict={G_input: Z_batch, y: y_batch, y_fill: y_batch_fill, is_training: True})
    ...
    for k in range(n):
        ...
        y_batch = np.zeros([n, 1, 1, 10])
        g = sess.run(G_sample, feed_dict={G_input: z_, y: y_batch, is_training: False})
        g = (g+1)/2
        # g = -1*(g-1)
        for j in range(n):
